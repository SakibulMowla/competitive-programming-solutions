...
int kmp()
{
    int i, j, k, len= SZ(strf);
    CLR(par);
    par[0]=k=0;

    for(i=1; i<len; i++)
    {
        while(k>0 && strf[k]!=strf[i])
            k=par[k-1];
    ...
    int cnt=0 , st= SZ(str2); myset.clear();
//    for(i=st+1; i<len; i++)
//    {
////        if(par[i] && strf[i]==str2[0])
////        {
//            int ind;
//            int x= (i- SZ(str2)) + (SZ(str2)-par[i]);
//            myset.insert(x);
//            ind= par[i]-1;
//            while(par[ind]>0)
//            {
//                int x= (i- SZ(str2)) + (SZ(str2)-par[ind]);
//                myset.insert(x);
//                ind= par[ind]-1;
//            }
//            deb(i ,x);
...
int main()
{
    int i, j, k, u, v, w, a, b, c, tcase, t, m, n, res;
    vi arr;
    t=0;
    cin>>tcase;
