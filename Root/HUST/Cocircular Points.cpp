...
bool kopa(double x1,double y1,double x2,double y2,double x3,double y3,double x4,double y4)
{
    double x,y,dx,dy,d,r,r1,dx1,dy1,dx4,dy4;
...
void process(int a,int b,int c)
{
    ...
    for(i=c+1; i<=n; i++)
        if(kopa(point[a].x,point[a].y,point[b].x,point[b].y,point[c].x,point[c].y,point[i].x,point[i].y))
            j++;
...
int main()
{
    ...
    while(scanf("%d",&n) == 1)
    {
        ...
        for(i=1; i<=n; i++)
            for(j=i+1; j<=n; j++)
                for(k=j+1; k<=n; k++)
                    if(check(point[i].x, point[i].y, point[j].x, point[j].y, point[k].x, point[k].y))
                        process(i, j, k);
