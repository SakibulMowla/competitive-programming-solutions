...
string Div_mod(string a,int k); //Modulus of Division a%k
...
string bigMod(string a, string b, long long m)
{
    if (b == "0") return "1";
    string x = bigMod(a, Division(b, 2), m);
    x = Div_mod(Multiplication(x, x), m);
    if (Div_mod(b, 2) == "1") x = Div_mod(Multiplication(x, a), m);
    return x;
}
...
string Div_mod(string a,int k)
{
    int i;
    long long sum=0;
    REP(i,SZ(a))
    sum=(sum*10+(a[i]-'0'))%k;
    return std::to_string((int)sum);
}
