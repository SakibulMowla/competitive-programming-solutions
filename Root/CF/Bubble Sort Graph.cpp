...
//    PR(b,e);
    ...
    heap[node] = FastMax(heap[node<<1], heap[(node<<1)|1]);
}
...
//    PR(f, t);
    if(f>t) return 0;
    if(b>=f && e<=t) return heap[node];
    int mid = (b+e) >> 1, ret = 0;
    if(f<=mid) ret = query(left, f, t);
    if(t>mid)  ret = FastMax(ret, query(riht, f, t));
    return ret;
}
...
//        freopen("2.txt", "w", stdout);
    ...
    while(scanf("%d",&n) == 1){
//        memset(heap, 0, sizeof(int)*(n+2));
        REP1(i, 1, n){
            RD(item);
            update(1, 1, n, item, query(1, 1, n, 1, item-1)+1);
        }
        PR(heap[1]);
    }
