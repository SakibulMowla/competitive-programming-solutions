...
string Multiplication(string a,ll k); //Multiplication between a and int k
...
int main()
{
    ...
    while(t--) {
        scanf("%s %lld %s", xx, &m, nn);
        x = xx, n = nn;
        m = m + 1LL;

        ans = power(x, m);
//        printf("OK\n");
        ans = Subtraction(ans, one);
        denum = Subtraction(x, one);
        ans = Division(ans, denum);
        ans = Div_mod(ans, n);
