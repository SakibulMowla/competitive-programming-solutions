...
const int MAX_LINE_LEN = 4000000;

char correct[2 * MAX_LINE_LEN + 5];
char returned[2 * MAX_LINE_LEN + 5];

vector<string> get_tokens(char *s) {
...
int main(int argc, char **argv) {
    oi::Scanner output(argv[2]);
    oi::Scanner answer(argv[3]);

    while (!output.isEOF()) {
        output.readLine(correct, 2 * MAX_LINE_LEN);
        answer.readLine(returned, 2 * MAX_LINE_LEN);
        if ((int) strlen(returned) > MAX_LINE_LEN) {
            cerr << "line too long" << endl;
            return 1;
        }
        vector<string> tokens1 = get_tokens(correct);
        vector<string> tokens2 = get_tokens(returned);
        if (tokens1.size() != tokens2.size()) {
            cerr << "wrong number of tokens" << endl;
            return 1;
        }
        if (tokens1[0] != tokens2[0]) {
            cerr << "different boolean expected" << endl;
            return 1;
        }
        ...
        vector<string>::iterator it = find(tokens2.begin(), tokens2.end(), tokens1[0]);
        if (it == tokens2.end()) {
            cerr << "cycle head not found" << endl;
            return 1;
        }
        int pos = it - tokens2.begin();
        if (tokens2[(pos + 1) % tokens2.size()] != tokens1[1]) {
            reverse(tokens2.begin(), it + 1);
            reverse(it + 1, tokens2.end());
        } else {
            reverse(tokens2.begin(), it);
            reverse(it, tokens2.end());
            reverse(tokens2.begin(), tokens2.end());
        }
        if (tokens1 != tokens2) {
            cerr << "bad cycle" << endl;
            return 1;
